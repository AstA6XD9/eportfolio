        ...
        >
          ...
          >
            {toolsData.map((tool, index) => {
              let toolSrc = tool;
              if (tool === assets.firebase && isDarkMode) toolSrc = assets.firebase_dark;
              else if (tool === assets.linux) toolSrc = isDarkMode ? assets.linux_dark : assets.linux;
              return (
                <motion.li
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
          ...
          >
            <h4 className={`mb-4 font-Ovo ${isDarkMode ? "text-gray-200" : "text-gray-700"}`}>
              {t.about.interests}
            </h4>
            <div className="flex flex-wrap gap-3">
              {interests.map(({ name }) => (
                <span
                  key={name}
                  className={`px-4 py-2 rounded-full text-sm font-medium border ${isDarkMode ? "border-gray-700 bg-gray-900 text-gray-200" : "border-gray-200 bg-gray-100 text-gray-700"}`}
                >
                  {name}
                </span>
              ))}
